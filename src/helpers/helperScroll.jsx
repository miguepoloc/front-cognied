class Scroll {
  ...
  isHover () {
    this.seccionesArray.forEach(({ id }) => {
      ...
      if (section && sectionNav && this.navBar) {
        const positionSectionTop =
                    section.offsetTop - this.navBar.offsetHeight
  ...
  scroll (id) {
    ...
    if (element && this.navBar) window.scrollTo(0, element.offsetTop - this.navBar.offsetHeight + 5)
    else if (element) window.scrollTo(0, element.offsetTop)
